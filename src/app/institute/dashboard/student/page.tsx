...
function Student(){
  ...
  const[searchedText,setSearchedText]=useState("")
    ...
    const filteredData=students.filter((student)=>{
      ...
      return(
        student.studentName.toLowerCase().includes(searchLower) || (student.id?.toString()??"").toLowerCase().includes(searchLower) || student.studentPhoneNo.toLowerCase().includes(searchLower) || student.studentAddress.toLowerCase().includes(searchLower) || student.enrolledDate.toLowerCase().includes(searchLower)
      )
    })
...
const closeEditModal = () => {
  ...
  <StudentEditModal student={selectedStudent} closeModal={closeEditModal} />
)}
                ...
                <table className=" min-w-full rounded-xl">
                  <thead>
                    <tr className="bg-gray-50">
                      ...
                      <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Mobile</th>
                      <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Address</th>
                      <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize rounded-t-xl">Enrolled</th>
                  ...
                  <tbody className="divide-y divide-gray-300 ">
                    {filteredData.length>0 && filteredData.map((student:IStudentData)=>(
                        <tr key={student.id}  className="bg-white transition-all duration-500 hover:bg-gray-50">
                      <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 ">{student?.id}</td>
                      <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{student.studentName}</td>
                      <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{student.studentPhoneNo}</td>
                      <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{student.studentAddress}</td>
                      <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{student.enrolledDate}</td>
                      
                      
                     
                      <td className=" p-5 ">
                        <div className="flex items-center gap-1">
                          <button onClick={() => openEditModal(student)} className="p-2  rounded-full  group transition-all duration-500  flex item-center">
