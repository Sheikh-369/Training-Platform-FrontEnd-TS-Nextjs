...
//   openEditModal
    const openEditCategoryModal = (category: ICategoryData) => {
      setSelectedCategory(category);
...
//     closeEditModal
  ...
  //searchFilteringWithRespectTo....
  const filteredData = category.filter((c) => {const searchLower = searchedText.toLowerCase();
  return (
    c.categoryName.toLowerCase().includes(searchLower) ||(c.id?.toString() ?? "").toLowerCase().includes(searchLower) ||c.categoryDescription.toLowerCase().includes(searchLower));
    });
  ...
  <div className=" overflow-x-auto">
    {/*add category modal */}
    {isModalOpen && instituteNumber && (
  <AddCategoryModal closeModal={closeModal} instituteNumber={instituteNumber} />)}

    {/* edit category modal */}
{editCategoryModalOpen && selectedCategory && instituteNumber && (
  <EditCategoryModal
      ...
      <div className="overflow-hidden ">
        <table className=" min-w-full rounded-xl">
          ...
          <tbody className="divide-y divide-gray-300 ">
            ...
            {filteredData.length > 0 && filteredData.map((c:ICategoryData) => {
              return(
            // JSX goes here
            <tr key={c?.id} className="bg-white transition-all duration-500 hover:bg-gray-50">
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 ">{c?.id}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{c.categoryName}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{c.categoryDescription}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{c.createdAt ? new Date(c.createdAt).toLocaleDateString() : "N/A"}</td>
              <td className=" p-5 ">
                <div className="flex items-center gap-1">
                    {/* Edit Button */}
                  <button onClick={() => openEditCategoryModal(c)} className="p-2  rounded-full  group transition-all duration-500  flex item-center">
                  ...
                  {showDeleteModal && selectedCategory && instituteNumber && (
                      <DeleteCategoryModal
                        category={selectedCategory}
