...
import { deleteTeacher, fetchTeacher } from "@/lib/store/institute/teacher/teacherSlice"
import { ITeacherData } from "@/lib/store/institute/teacher/teacherSliceTypes"
import { useEffect, useState } from "react"
import InstituteTeacherModal from "./addTeacherModal"
import TeacherEditModal from "./editTeacherModal"

function Teacher(){
  const [searchedText,setSearchedText]=useState("")//search part

  const [editTeacherModalOpen, setEditTeacherModalOpen] = useState(false);//edit part
  const [selectedTeacher, setSelectedTeacher] = useState<ITeacherData | null>(null);//edit part
    ...
    const filteredData=teachers.filter((teacher)=>{
      ...
      return(
        teacher.teacherName.toLowerCase().includes(searchLower) || (teacher.id?.toString()??"").toLowerCase().includes(searchLower) || teacher.teacherExpertise.toLowerCase().includes(searchLower) || teacher.teacherPhoneNumber.toLowerCase().includes(searchLower) || teacher.teacherSalary.toLowerCase().includes(searchLower)
      )
    })


    //edit part
    const openEditTeacherModal = (teacher: ITeacherData) => {
  ...
  <div className=" overflow-x-auto">
    {/* add teacher modal */}
    {isModalOpen && <InstituteTeacherModal closeModal={closeModal}/>}
    {/* edit teacher modal */}
    {editTeacherModalOpen && selectedTeacher && (
    <TeacherEditModal teacher={selectedTeacher} closeModal={closeEditTeacherModal} />
    )}
    <div className="min-w-full inline-block align-middle">
      <div className="flex justify-between relative  text-gray-500 focus-within:text-gray-900 mb-4">
        ...
        <input onChange={(e)=>setSearchedText(e.target.value)} type="text"   id="default-search" className="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search for teacher" />
        <button onClick={openModal} className="bg-green-500 rounded p-1 text-white cursor-pointer">+Teacher</button>
      </div>
        ...
        <table className=" min-w-full rounded-xl">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Mob </th>
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Expertise </th>
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize"> Joined </th>
          ...
          <tbody className="divide-y divide-gray-300 ">
            {filteredData.length > 0 && filteredData.map((teacher:ITeacherData)=>(
              ...
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{teacher.teacherExpertise}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{teacher.teacherJoinDate}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{teacher.teacherSalary}</td>
              <td className=" p-5 ">
                <div className="flex items-center gap-1">
                  <button onClick={() => openEditTeacherModal(teacher)} className="p-2  rounded-full  group transition-all duration-500  flex item-center">
                  ...
                  <button onClick={()=>handleTeacherdelete(teacher.id)} className="p-2 rounded-full  group transition-all duration-500  flex item-center">
