...
import { deleteCourse, fetchCourse } from "@/lib/store/institute/course/courseSlice"
...
function Course(){
  ...
  //to fetch course
  useEffect(()=>{
    dispatch(fetchCourse())//to display course details in the edit modal(pop up)
    dispatch(fetchTeacher())//to display teacher in the edit modal(pop up)
    dispatch(fetchCategory())//to display category in the edit modal(pop up)
  },[])
  ...
  //for search logic
  const filteredData=course.filter((data)=>{
    const searchLower=searchedText.toLowerCase()
    return(
        data.courseName.toLowerCase().includes(searchLower) || (data.id?.toString()??"").toLowerCase().includes(searchLower) || data.courseLevel.toLowerCase().includes(searchLower) || data.categoryName.toLowerCase().includes(searchLower) || data.teacherName.toLowerCase().includes(searchLower) || data.coursePrice.toLowerCase().includes(searchLower)
      )
  })
  ...
  <div className=" overflow-x-auto">
    ...
    <div className="min-w-full inline-block align-middle">
      <div className="flex justify-between relative  text-gray-500 focus-within:text-gray-900 mb-4">
        ...
        <input onChange={(e)=>setSearchedText(e.target.value)} type="text" id="default-search" className="block w-80 h-11 pr-5 pl-12 py-2.5 text-base font-normal shadow-xs text-gray-900 bg-transparent border border-gray-300 rounded-full placeholder-gray-400 focus:outline-none" placeholder="Search for course" />
        ...
        <table className=" min-w-full rounded-xl">
          <thead>
            <tr className="bg-gray-50">
              ...
              <th scope="col" className="p-5 text-left text-sm leading-6 font-semibold text-gray-900 capitalize">Fee</th>
          ...
          <tbody className="divide-y divide-gray-300 ">
            {filteredData.length>0 && filteredData.map((course:ICourseData)=>(
            <tr key={course.courseName} className="bg-white transition-all duration-500 hover:bg-gray-50">
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900 ">{course?.id}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{course?.courseName}</td>
              <td className="p-5 whitespace-nowrap text-sm leading-6 font-medium text-gray-900">{course?.coursePrice}</td>
              ...
              <td className=" p-5 ">
                <div className="flex items-center gap-1">
                  <button onClick={() => openEditCourseModal(course)} className="p-2  rounded-full  group transition-all duration-500  flex item-center">
