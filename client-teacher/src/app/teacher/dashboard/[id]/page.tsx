...
const TeacherPersonPage = () => {
  ...
  const { instituteNumber } = useAppSelector((store) => store.auth.authData);

  const params = useParams();
  const id = params.id;

useEffect(() => {
  if (id && instituteNumber) {
    dispatch(fetchTeacherData(id as string, String(instituteNumber)));
  }
}, [dispatch, id, instituteNumber]);
  ...
  return (
    <div className="p-6 bg-slate-800 rounded-lg shadow-md">
      {/* Optional: Uncomment if you implement Edit Modal */}
      {editTeacherDataModal && editTeacherData && id && instituteNumber && (
  <EditTeacherModal
    teacher={editTeacherData}
    closeModal={closeEditTeacherDataModal}
    teacherId={id as string}
    instituteNumber={String(instituteNumber)} // ✅ added this line
  />
)}
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="md:col-span-1 flex justify-center">
          <div className="flex flex-col items-center">
            ...
            <p className="font-medium text-white text-4xl mt-4">
              {teacher?.teacherName}
            </p>
          </div>
        </div>
        ...
        <div className="md:col-span-2 bg-slate-700 p-6 rounded-lg shadow-md flex flex-col h-full">
          ...
          <div className="mt-6">
            ...
            <p className="text-slate-200 leading-relaxed">{teacher?.aboutTeacher}</p>
          </div>

          {/* Edit Profile Button */}
          <div className="mt-6 flex justify-end">
            <button
              onClick={() => openEditTeacherDataModal(teacher!)}
